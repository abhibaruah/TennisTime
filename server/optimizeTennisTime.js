...
function organizeResultsByDate(paretoFront) {
  ...
  paretoFront.forEach(slot => {
    ...
    resultsByDate[slot.Date].push({
      ...
      paretoScore: slot.overallScore // Include the Pareto score in the response
    });
  });
...
function optimizeTennisTimes(filePath, returnData = false) {
  try {
    ...
    // Filter out Pareto times from all valid times to avoid duplication
    const paretoIds = new Set(paretoFront.map(slot => `${slot.Date}-${slot.Time}`));
    const nonParetoValidTimes = scoredTimeSlots.filter(
      slot => !paretoIds.has(`${slot.Date}-${slot.Time}`)
    );
    
    // Organize results by date for API response
    const organizedParetoResults = organizeResultsByDate(paretoFront);
    
    // Organize non-Pareto valid time slots by date
    const organizedNonParetoValidResults = organizeResultsByDate(nonParetoValidTimes);
    ...
    return {
      paretoTimes: organizedParetoResults,
      nonParetoValidTimes: organizedNonParetoValidResults
    };
  } catch (error) {
